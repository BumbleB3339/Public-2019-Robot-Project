/*----------------------------------------------------------------------------*/
/* Copyright (c) 2018 FIRST. All Rights Reserved.                             */
/* Open Source Software - may be modified and shared by FRC teams. The code   */
/* must be accompanied by the FIRST BSD license file in the root directory of */
/* the project.                                                               */
/*----------------------------------------------------------------------------*/

package frc.robot.commands.intake_arm;

import edu.wpi.first.wpilibj.command.Command;
import frc.robot.Robot;
import frc.robot.subsystems.IntakeArm.IntakeArmState;

public class SetIntakeState extends Command {

  private IntakeArmState desiredState;
  private boolean waitForOnTarget;

  public SetIntakeState(IntakeArmState desiredState) {
    this(desiredState, true);
  }

  public SetIntakeState(IntakeArmState desiredState, boolean waitForOnTarget) {
    requires(Robot.m_intakeArm);
    this.desiredState = desiredState;
    this.waitForOnTarget = waitForOnTarget;
  }

  // Called just before this Command runs the first time
  @Override
  protected void initialize() {
    Robot.m_intakeArm.setPosition(desiredState);
  }

  // Called repeatedly when this Command is scheduled to run
  @Override
  protected void execute() {
  }

  // Make this return true when this Command no longer needs to run execute()
  @Override
  protected boolean isFinished() {
    if (waitForOnTarget) {
      return Robot.m_intakeArm.isOnTarget();
    } else {
      return true;
    }
  }

  // Called once after isFinished returns true
  @Override
  protected void end() {
  }

  // Called when another command which requires one or more of the same
  // subsystems is scheduled to run
  @Override
  protected void interrupted() {
  }
}
