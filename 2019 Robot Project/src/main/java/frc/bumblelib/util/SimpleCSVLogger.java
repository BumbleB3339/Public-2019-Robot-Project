...
public class SimpleCSVLogger {
	...
	public int init(String[] data_fields, String[] units_fields) {
		...
		try {
			...
			log_name = output_dir + logName + "_" + getDateTimeString() + ".csv";
	...
	public int init(ArrayList<String> data_fields, ArrayList<String> units_fields) {
	...
	public int writeCSVString(String CSVString, boolean createNewLineAtEnd) {
		...
		try {
			...
			// Write user-defined data
			line_to_write = CSVString;

			// End of line
			if (createNewLineAtEnd) {
				line_to_write = line_to_write.concat("\n");
			}
	...
	private String getDateTimeString() {
		...
		df.setTimeZone(TimeZone.getTimeZone("UTC+02"));
